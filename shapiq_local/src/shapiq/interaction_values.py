...
class InteractionValues:
    ...
    def save(self, path: Path, **kwargs) -> None:
        ...
        Args:
            ...
            **kwargs: Additional metadata to store in the JSON file (e.g. runtime info).
        """
...
def _validate_and_return_interactions(
    ...
    if isinstance(values, dict):
        # Sparse fast path: dict already maps tuples to values. Skip
        # generate_interaction_lookup, which would build a dense dict of all
        # C(n_players, k) tuples up to max_order — OOM for large n at higher
        # orders. dict(values) is enough: keys are tuples and values are
        # floats, both immutable, so a shallow copy is safe against later
        # in-place mutations of the returned interactions dict.
