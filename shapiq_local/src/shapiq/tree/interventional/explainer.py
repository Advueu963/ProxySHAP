...
class InterventionalTreeExplainer:
    ...
    * **Sparse path** — :func:`compute_interactions_batched_sparse` for
    ...
    For tree ensembles classified by ``predict_proba``, the baseline value is
    taken on the model's logit / raw-score scale (XGBoost / LightGBM) or on
    ...
    Attributes:
        ...
        baseline_value: Mean prediction on ``reference_data`` (scalar);
    ...
    def explain_function(
        ...
        **_: dict,
        ...
        docstring). The order-0 entry is set to ``self.baseline_value``.
        ...
        Returns:
            :class:`~shapiq.interaction_values.InteractionValues` with
            ``min_order=1``, ``max_order=self.max_order`` and the requested
            interaction index.
        """
        ...
        interactions = {}
        # For higher order interactions we need to use the sparse implementation as the flatten one is only optimized for main effects, pairwise, and triple interactions. For orders up to 3, we can use the flatten implementation which is faster.
