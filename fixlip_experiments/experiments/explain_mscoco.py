...
if __name__ == "__main__":
    ...
    # BUDGET_RANGE = [100, 200, 500, 1000, 2000, 10000]
    ...
    for i, d in enumerate(dataset):
            ...
            if game.n_players_image == 49:
                ...
                if MODE == "banzhaf":
                    ...
                    while True:
                        ...
                        crossmodal_budget += 1
                    print("Found crossmodal budget:", crossmodal_budget, "with split:", fixlip_attribution.split_budget(crossmodal_budget), flush=True)
                ...
                if MODE == "banzhaf":
                    ...
                    #     crossmodal_budget += 1
                    # print("Found crossmodal budget:", crossmodal_budget, "with split:", fixlip_attribution.split_budget(crossmodal_budget), flush=True)
                    # if crossmodal_budget > 1_000_000:
                    #     print("Crossmodal budget too large, skipping...", flush=True)
                    #     continue
                    # budget_to_crossmodal[(i, BUDGET)] = crossmodal_budget
                    # attribution_values = fixlip_attribution.approximate_crossmodal(
                    #     game=game, budget=crossmodal_budget
                    ...
                    # )
                    # interaction_values = fixlip_interaction.approximate_crossmodal(
                    #     game=game, budget=crossmodal_budget, interaction_lookup=interaction_lookup
                    # )
                elif MODE == "shapley":
                    fixlip_attribution = src.fixlip.FIxLIP(
                        n_players=game.n_players,
            ...
            if budget_to_crossmodal.get((i, BUDGET), None) is not None:
                attribution_values_save_name = f"/iv_order1_{i}_{BUDGET}_{APPROXIMATION_TYPE}_crossmodal.json"
                attribution_fig_save_name = f"/ex_order1_{i}_{BUDGET}_{APPROXIMATION_TYPE}_crossmodal.png"
                interaction_values_save_name = f"/iv_order2_{i}_{BUDGET}_{APPROXIMATION_TYPE}_crossmodal.json"
                interaction_fig_save_name = f"/ex_order2_{i}_{BUDGET}_{APPROXIMATION_TYPE}_crossmodal.png"
            else:
                attribution_values_save_name = f"/iv_order1_{i}_{BUDGET}_{APPROXIMATION_TYPE}_{size_clique}.json"
                attribution_fig_save_name = f"/ex_order1_{i}_{BUDGET}_{APPROXIMATION_TYPE}_{size_clique}.png"
                interaction_values_save_name = f"/iv_order2_{i}_{BUDGET}_{APPROXIMATION_TYPE}_{size_clique}.json"
                interaction_fig_save_name = f"/ex_order2_{i}_{BUDGET}_{APPROXIMATION_TYPE}_{size_clique}.png"
            

            attribution_values.save(
                Path(
                    PATH_OUTPUT + attribution_values_save_name
                ),
                **fixlip_attribution.runtime_last_approximate_run,
                game_split=fixlip_attribution.split_budget(BUDGET),
                crossmodal_budget=budget_to_crossmodal.get((i, BUDGET), None),
                # os.path.join(
            ...
            interaction_values.save(
                Path(
                    PATH_OUTPUT + interaction_values_save_name
                ),
            ...
            # )
            # fig.savefig(
            #     os.path.join(PATH_OUTPUT, interaction_fig_save_name), bbox_inches="tight"
            # )
            # plt.close(fig)
            # fig = src.plot.plot_image_and_text_together(
            #     img=input_image_denormalized,
            ...
            # )
            # fig.savefig(
            #     os.path.join(PATH_OUTPUT, attribution_fig_save_name), bbox_inches="tight"
            # )
            # plt.close(fig)
            # ##
