...
class MonteCarlo(Approximator[TIndices]):
    ...
    def approximate_given(
        ...
        sampling_adjustment_weights: np.ndarray | dict[tuple[int, ...], np.ndarray],
        **kwargs: Any,  # noqa: ARG002
    ) -> InteractionValues:
        """Approximate interaction values on a pre-sampled coalition batch.

        This bypasses internal coalition sampling and directly consumes externally
        provided coalition/value samples together with their sampling-adjustment
        weights.

        Args:
            coalitions_matrix: Binary coalition matrix in shape ``(n_coalitions, n)``.
            game_values: Game values corresponding to ``coalitions_matrix``.
            sampling_adjustment_weights: Adjustment weights in shape
                ``(n_coalitions,)`` for unbiased Monte Carlo estimation.
            **kwargs: Additional keyword arguments (unused).

        Returns:
            Approximated interaction values.
        """
        budget = int(coalitions_matrix.shape[0])
        t_start = time.perf_counter()
    ...
    def monte_carlo_routine(
        ...
        sampling_adjustment_weights: (
            np.ndarray | dict[tuple[int, ...], np.ndarray] | None
        ) = None,
    ) -> np.ndarray:
        """Approximates the Shapley interaction values using Monte Carlo sampling.
        ...
        empty_coalition_value = float(game_values[0])
        ...
        for interaction, interaction_pos in self.interaction_lookup.items():
            ...
            if isinstance(sampling_adjustment_weights, dict):
                current_sampling_adjustment_weights = sampling_adjustment_weights.get(
                    interaction,
                    np.ones(coalitions_matrix.shape[0], dtype=float),
                )
            elif sampling_adjustment_weights is not None:
                current_sampling_adjustment_weights = sampling_adjustment_weights
            elif self.stratify_coalition_size and self.stratify_intersection:  # this is SVARM-IQ
                current_sampling_adjustment_weights = self._svarmiq_routine(interaction)
            elif not self.stratify_coalition_size and self.stratify_intersection:
                current_sampling_adjustment_weights = self._intersection_stratification(interaction)
            elif self.stratify_coalition_size and not self.stratify_intersection:
                current_sampling_adjustment_weights = self._coalition_size_stratification()
            else:  # this is SHAP-IQ
                current_sampling_adjustment_weights = self._shapiq_routine()
            ...
            shapley_interaction_values[interaction_pos] = np.sum(
                game_values_centered * interaction_weights * current_sampling_adjustment_weights,
            )
