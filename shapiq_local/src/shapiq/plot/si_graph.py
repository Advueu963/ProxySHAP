    ...
    *,
    ...
    if n_interactions is not None:
        ...
        interaction_values = interaction_values.get_top_k_interactions(
            n_interactions
        )  # TODO: Was get_top_k(n_interactions) which should be wrong. # noqa: TD003
    ...
    adjusted_pos: dict = {}
    if pos is None:
        # TODO: pos is statically just a Mapping. Forcing it to be dict is way stronger but necessary as far I see it # noqa: TD003
        if circular_layout:
...
# TODO: This function is not used at all. If not given an axis it will also crash. What is the meaning of this function # noqa: TD003
...
def _draw_fancy_hyper_edges(
    ...
    for hyper_edge in hyper_edges:
        ...
        if len(hyper_edge) == 2:
            ...
            # TODO: Technically there is not guarantee it is not sure that the hyper_edge must exist # noqa: TD003
...
def _draw_explanation_nodes(
    ...
    for node in graph.nodes:
        ...
        if nodes is not None and node not in nodes:
            continue
        position = pos[node]
        # TODO: Statically it seems to be not clear that we get an object which is subscriptable # noqa: TD003
