...
class Approximator(ABC, Generic[TIndices]):
    ...
    def __init__(
        ...
        *,
        ...
        self.runtime_last_approximate_run: dict[str, float] = {"evaluations": 0.0, "total": 0.0}
    ...
    def _make_timed_game(self, game):
        """Wraps game callable to accumulate wall-time spent in evaluations.

        Returns (timed_game, get_elapsed) — call timed_game in place of game,
        then call get_elapsed() after all calls to get total seconds.
        """
        eval_time = 0.0

        def timed_game(coalitions):
            nonlocal eval_time
            t0 = time.perf_counter()
            result = game(coalitions)
            eval_time += time.perf_counter() - t0
            return result

        return timed_game, lambda: eval_time
