...
def get_top_clique_players(attribution_values, size_clique, n_players_image, n_players_text):
    ...
    # print(f"Selecting clique of size {size_clique} with {size_clique_image} image and {size_clique_text} text players.", flush=True)
    # print(f"Total players: image {n_players_image}, text {n_players_text}.", flush=True)
    # print(f"Shape of get_n_order(1) image: {av_image.get_n_order(1).values.shape}", flush=True)
    # print(f"Original av text values: {av_text.interactions}", flush=True)
    # print(f"Av text values after get_n_order: {av_text.get_n_order(1).interactions}", flush=True)
    # print(f"Shape of get_n_order(1) text: {av_text.get_n_order(1).values.shape}", flush=True)
    # print(f"{-size_clique_text} is causing the error", flush=True)
