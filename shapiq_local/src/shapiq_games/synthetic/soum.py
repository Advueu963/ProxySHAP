...
class SOUM(Game):
    ...
    def __init__(
        ...
        *,
        ...
        # Precomputed matrix for vectorized value_function.
        # Row k = interaction_binary of basis game k; 1 means player is required.
        self._interactions_matrix = np.array(
            [g.interaction_binary for g in self.unanimity_games.values()], dtype=np.int8
        )  # (n_basis_games, n_players)
        self._interaction_sizes = self._interactions_matrix.sum(axis=1)  # (n_basis_games,)
    ...
    def value_function(self, coalitions: np.ndarray) -> np.ndarray:
        ...
        Returns:
            The worth of the coalition.
        """
        # overlap[i, k] = number of required players of basis game k present in coalition i.
        # contained[i, k] is True iff coalition i includes all players needed by basis game k.
        overlap = coalitions.astype(np.int8) @ self._interactions_matrix.T
        contained = overlap == self._interaction_sizes
        return contained.astype(np.float64) @ self.linear_coefficients
