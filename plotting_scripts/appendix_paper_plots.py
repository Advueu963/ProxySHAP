...
if __name__ == "__main__":
    # Datasets shown one per row; columns are SII-2, BII-2, SII-3, BII-3.
    DATASETS_A4 = [
        ("RealEstateLocalXAI", "tabpfn"),
        ("ThyroidLocalXAI", "interventional"),
        ("IonosphereLocalXAI", "interventional"),
        ("TabArenaDiabetes130usLocalXAI", "interventional"),
        ("TabArenaSpliceLocalXAI", "interventional"),
        ("TabArenaSuperconductivityLocalXAI", "interventional"),
        ("TabArenaMicLocalXAI", "interventional"),
        ("TabArenaApsFailureLocalXAI", "interventional"),
    ]

    # Column layout (row-major): col0=SII-2, col1=BII-2, col2=SII-3, col3=BII-3.
    COLUMN_SPECS = [
        ("SII", 2),
        ("BII", 2),
        ("SII", 3),
    ...
    # Panels where ProxySHAP (XGBoost, MSR) should be hidden
    # (Crime + APSFailure, BII order 3).
    EXCLUDE_PER_PANEL = {
        ("CommunitiesAndCrimeLocalXAI", "BII", 3): ["ProxySHAP (XGBoost, MSR) [our]"],
        ("TabArenaApsFailureLocalXAI", "BII", 3): ["ProxySHAP (XGBoost, MSR) [our]"],
    }

    plot_specs_a4 = []
    for game_name, game_type in DATASETS_A4:
        for index, order in COLUMN_SPECS:
            spec = {
                "game_name": game_name,
                "order": order,
                "index": index,
            ...
            exclude = EXCLUDE_PER_PANEL.get((game_name, index, order))
            if exclude:
                spec["exclude_approximators"] = exclude
            plot_specs_a4.append(spec)

    # Union of CSV templates from the previous SII and BII batteries.
    data_path_template = [
        "results_benchmark_{index}_{order}_{game_type}.csv",
        "results_benchmark_{index}_{order}_{game_type}_local.csv",
        "results_benchmark_{index}_{order}_{game_type}_local2.csv",
        "results_benchmark_{index}_{order}_{game_type}_sii3_missing.csv",
        "results_benchmark_{index}_{order}_{game_type}_bii3_missing.csv",
        "results_benchmark_{index}_{order}_{game_type}_sii3_permutation2.csv",
        "results_benchmark_{index}_{order}_{game_type}_sii2_permutation2.csv",
        "results_benchmark_{index}_{order}_{game_type}_sii2_permutation_missing.csv",
        "results_benchmark_{index}_{order}_{game_type}_sii3_permutation_missing.csv",
    ...
    plot_combined_main_paper_plots(
        ...
        shared_xlabel_y=0,
        shared_ylabel_fontsize=16,
        shared_xlabel_fontsize=16,
        ...
        column_headers=["SII", "BII", "SII", "BII"],
        column_group_headers=[("Order 2", 0, 1), ("Order 3", 2, 3)],
        row_title_on_right=True,
        linewidth=1.5,
        auto_ylim=True,
        data_path_template=data_path_template,
        output_path=f"{output_dir_a4}appendix_mse_battery_combined.pdf",
        y_log_scale=True,
        x_log_scale=True,
        marker_size=3,
        highlight_size=2,
