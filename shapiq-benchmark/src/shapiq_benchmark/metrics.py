...
def compute_diff_metrics(
    ...
    return [
        ...
        Metric(
            metric_id="RelativeMSE", value=(np.sum(diff_values**2) / np.sum(ground_truth.values**2))
        ),
    ]
...
def compute_kendall_tau(
    ...
    ground_truth_interactions = ground_truth.interactions
    estimated_interactions = estimated.interactions

    gt_values = []
    estimated_values = []
    for interaction in ground_truth_interactions.keys() | estimated_interactions.keys():
        gt_values.append(ground_truth_interactions.get(interaction, 0.0))
        estimated_values.append(estimated_interactions.get(interaction, 0.0))
...
def compute_spearmans_correlation(
    ...
    spearman_corr, _ = spearmanr(gt_values, estimated_values)
    # # array conversion
...
def get_all_metrics(
    ...
    """
    #´return compute_diff_metrics(ground_truth, estimated)
    metrics = [
